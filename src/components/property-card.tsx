...
export function PropertyCard({
  ...
  return (
    <article className={cn("group relative flex flex-col bg-card shadow-card", className)}>
      <Link
        to="/proprietes/$slug"
      ...
      >
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          <span className="bg-navy/90 px-3 py-1.5 text-[0.6rem] tracking-[0.18em] text-white uppercase backdrop-blur">
            {property.transaction === "vente" ? "À vendre" : "À louer"}
          </span>
          {property.isNew ? (
            <span className="bg-gold px-3 py-1.5 text-[0.6rem] tracking-[0.18em] text-navy uppercase">
              Nouveau
            </span>
          ) : null}
        </div>
